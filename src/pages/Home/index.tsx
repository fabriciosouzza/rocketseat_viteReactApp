...
import { Card, PropsOfCards } from '../../components/Card'

type GitProfileResponse = {
  name: string
}
...
export function Home() {
  ...
  const [studentName, setStudentName] = useState<string>(); //hook que atualiza a informação de studentName utilizando a função setStudentName
  const [students, setStudents] = useState<PropsOfCards[]>([]); //hook que atualiza a informa de students utilizando a função setStudents
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const data = await response.json() as GitProfileResponse;
      setUser({name: data.name})
    }
    fetchData()
    // fetch('https://api.github.com/users/mojombo')
    // .then(response => response.json() as GitProfileResponse)
    // .then(data => )
  }, []);
